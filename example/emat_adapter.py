...
parser.add_argument(
    '-a', type=int, help='angle start')
parser.add_argument(
    '-b', type=int, help='angle end')
parser.add_argument(
    '-i', type=int, help='increment')
parser.add_argument(
    '-m', type=int, default=1024, help='number of rows')
parser.add_argument(
    '-n', type=int, default=1024, help='number of columns')
...
if not args.prerecorded:
    ...
    count = len(angles)
    files = list(map(lambda ang: (ang, path + "/" + str(ang) + "_1.ser"), angles))
else:
    def to_float(str):
        return float(str[:-2])
